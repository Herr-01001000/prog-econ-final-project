...
if __name__ == "__main__":
    ...
    data = pd.read_csv(ppj("IN_DATA", "SAINC1_1998_2017_ALL_AREAS.csv"), encoding="ISO-8859-1")
    ...
    data = pd.wide_to_long(data, ['20'], i=['GeoName','LineCode'], j='year').reset_index()
    data = data.set_index(['GeoName', 'year', 'LineCode']).unstack().reset_index()
    data.columns = data.columns.map('{0[0]}{0[1]}'.format)
    
    data.rename(columns={'GeoName': 'state', '201.0': 'pi',
                         '202.0': 'population', '203.0': 'pi_pc'}, inplace=True)
