/*
In the file "reg2_compare.do", we regress the potential FinTech drivers on
...
// Regression 2: Comparison between FinTech and Traditional Lending
eststo clear
...
esttab est* using "${PATH_OUT_ANALYSIS}/`1'.tex", booktabs ///
	prehead("\tabularnewline\hline" "Dep. variable: &\multicolumn{5}{c}{Lending Volume} \\") ///
	postfoot("\bottomrule") ///
	unstack cells(b(star fmt(4)) p(par fmt(4))) ///
	mtitles("FinTech" "Banking" "Total Diff" "Credit Banking" "Credit Diff") ///
	numbers collabels(none) varlabels(_cons Constant) ///
	prefoot("\midrule" "Time FE & Yes & Yes & Yes & Yes & Yes \\" ///
		"State FE & Yes & Yes & Yes & Yes & Yes \\" "\midrule") ///
