...
to define globals:
    ...
    * ${VCE} - standard error cluster variables
...
// Read in model specs
do `"${PATH_OUT_MODEL_SPECS}/reg1_panel"'
...
esttab est* using "${PATH_OUT_ANALYSIS}/reg1_panel.tex", booktabs ///
	prehead("toprule" "Dep. variable: &\multicolumn{4}{c}{FinTech Lending} \\") ///
	postfoot("bottomrule") ///
	unstack cells(b(star fmt(4)) p(par fmt(4))) ///
	mtitles("amount" "amount" "number" "number") numbers ///
